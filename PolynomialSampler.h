...
public:
    ...
    std::vector<double> mod_q(const std::vector<int64_t>& coeffs, int64_t q);//将给定系数向量进行取模运算，其中超过 q/2 的系数被映射到 (-q/2, 0] 的范围内
    Polynomial<double> DG();//DG(sigma^2)表示从离散高斯分布中采样N个系数从而获得多项式e属于Rq(sigma=3)
    Polynomial<double> UniformPolynomial(int64_t q=-1);//Rq的N维多项式
